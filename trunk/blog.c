...
//    for (i = 0; i < 0x15; ++i)
//        check_buf[i] = Alog (RuijieExtra[i]);

    for (i = 0; i < 0x15; ++i) {
//        uint8_t index = h_byte ^ check_buf[i];
        uint8_t index = h_byte ^ Alog (RuijieExtra[i]);
        h_byte = Table[index * 2 + 1] ^ l_byte;
        l_byte = Table[index * 2];
    }
    RuijieExtra[0x15] = Alog (h_byte);
    RuijieExtra[0x16] = Alog (l_byte);
}
